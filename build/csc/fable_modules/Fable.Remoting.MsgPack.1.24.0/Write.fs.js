import { disposeSafe, isDisposable, getEnumerator, count, defaultOf } from "../fable-library.4.9.0/Util.js";
import { addToDict, tryGetValue } from "../fable-library.4.9.0/MapUtil.js";
import { name, class_type, option_type, list_type, obj_type, equals, getGenerics, getGenericTypeDefinition, isGenericType, isEnum, getTupleFields, getTupleElements, isTuple, getUnionCaseFields, getUnionFields, isUnion, getElementType, isArray, getRecordFields, getRecordElements, isRecord, fullName } from "../fable-library.4.9.0/Reflection.js";
import { FSharpRef } from "../fable-library.4.9.0/Types.js";
import { getBytesDouble, getBytesSingle, getBytesInt64, isLittleEndian } from "../fable-library.4.9.0/BitConverter.js";
import { head, map, reverse, addRangeInPlace } from "../fable-library.4.9.0/Array.js";
import { fromFloat64, toByteArray, fromUInt16, fromUInt32, fromInt16, fromInt32, toInt64, fromInt64, op_RightShift, toUInt64, toUInt8, compare } from "../fable-library.4.9.0/BigInt.js";
import { getBits } from "../fable-library.4.9.0/Decimal.js";
import { value as value_7 } from "../fable-library.4.9.0/Option.js";
import { printf, toFail } from "../fable-library.4.9.0/String.js";
import { get_UTF8 } from "../fable-library.4.9.0/Encoding.js";
import { guidToArray } from "../fable-library.4.9.0/Guid.js";
import { kind, getTicks } from "../fable-library.4.9.0/Date.js";
import { ticks, totalMinutes } from "../fable-library.4.9.0/TimeSpan.js";
import { offset } from "../fable-library.4.9.0/DateOffset.js";
import { dayNumber } from "../fable-library.4.9.0/DateOnly.js";
    ...
    if (x == null) {
        ...
        if (matchValue[0]) {
            ...
            cacheGetOrAdd(t, (x_7, out_9) => {
                writeInt64(toInt64(fromInt32(x_7)), out_9);
            })(x, out);
        }
...
addToDict(serializerCache, "System.SByte", (x, out) => {
    let value;
    writeByte((value = (x | 0), value & 0xFF), out);
});
...
addToDict(serializerCache, "System.Char", (x, out) => {
    ...
    const str = x;
    const out_1 = out;
    const str_1 = get_UTF8().getBytes(str);
    if (str_1.length < 32) {
...
addToDict(serializerCache, "System.Int32", (x, out) => {
    writeInt64(toInt64(fromInt32(x)), out);
});

addToDict(serializerCache, "System.Int16", (x, out) => {
    writeInt64(toInt64(fromInt16(x)), out);
});
...
addToDict(serializerCache, "System.UInt32", (x, out) => {
    writeUInt64(toUInt64(fromUInt32(x)), out);
});

addToDict(serializerCache, "System.UInt16", (x, out) => {
    writeUInt64(toUInt64(fromUInt16(x)), out);
});
...
addToDict(serializerCache, "System.Numerics.BigInteger", (x, out) => {
    writeBin(toByteArray(x), out);
});

addToDict(serializerCache, "System.Guid", (x, out) => {
    writeBin(guidToArray(x), out);
});
...
addToDict(serializerCache, "System.DateOnly", (x, out) => {
    let n, b, value, b_1, value_1, b_2, value_2, b_3;
    const date = x;
    ((n = (dayNumber(date) >>> 0), (b = ((value = (n >>> 24), value & 0xFF)), (b_1 = ((value_1 = (n >>> 16), value_1 & 0xFF)), (b_2 = ((value_2 = (n >>> 8), value_2 & 0xFF)), (b_3 = (n & 0xFF), (writeFormat) => {
        const b1 = b;
        const b2 = b_1;
        const b3 = b_2;
        const b4 = b_3;
...
addToDict(serializerCache, "System.TimeOnly", (x, out) => {
    writeUInt64(toUInt64(fromInt64(ticks(x))), out);
});

addToDict(serializerCache, "System.TimeSpan", (x, out) => {
    writeInt64(ticks(x), out);
});
